    ...
    private void Walking()
    {
        if (!playerController.inAir)
        {
            if (playerController.walkForward)
            {
                ...
                if (playerController.playerParts[11].transform.position.z < playerController.playerParts[12].transform.position.z && !playerController.stepLeft && !playerController.alert_Leg_Right)
                {
                    playerController.stepRight = true;
                    playerController.alert_Leg_Right = true;
                    playerController.alert_Leg_Left = true;
                }

                //Checks if the left leg is behind
                if (playerController.playerParts[11].transform.position.z > playerController.playerParts[12].transform.position.z && !playerController.stepRight && !playerController.alert_Leg_Left)
                {
